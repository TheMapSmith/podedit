...
must_haves:
  ...
  artifacts:
    ...
    - path: "index.html"
      ...
      contains: "cancel-processing-btn"
  key_links:
    - from: "index.html"
      to: "AudioProcessingService.cancel"
      via: "click handler"
      pattern: "audioProcessingService.cancel"
---
...
5. Add specific error handling for cancel in the catch block:
  ...
  <verify>grep -q "cancel()" src/services/audioProcessingService.js && grep -q "cancelRequested" src/services/audioProcessingService.js && echo "Cancel method and flag exist"</verify>
...
6. Handle cancel error in catch block - show as info (blue/neutral) not error (red):
   - Check if error.message includes 'cancelled' (case-insensitive)
   - If cancelled: use info styling (blue background #e7f3ff, border #007bff, text #004085)
   - Display message: "Processing cancelled"
   - If NOT cancelled: use error styling (red background #f8d7da, border #dc3545, text #721c24)
   - Display the actual error.message

   Example catch block update:
   ```javascript
   } catch (error) {
  ...
  <verify>grep -q "cancel-processing-btn" index.html && grep -q "audioProcessingService.cancel" index.html && echo "Cancel button and handler exist"</verify>
  <done>Cancel button visible during processing, click aborts operation, UI returns to ready state with appropriate status styling</done>
</task>
...
3. Click cancel
4. Processing stops
5. Status shows "Processing cancelled" (info styling, not error styling)
