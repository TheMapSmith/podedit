...
1. Add estimation function in script (note: browserCompat is already available in script scope from earlier initialization):
   ```javascript
   /**
...
5. Show estimate when Export Edited Audio button is clicked (before processing starts):
   - Calculate estimate using estimateProcessingTime(duration, currentFile.size, browserCompat)
...
7. Hide estimate in finally block:
  ...
  <verify>grep -q "processing-estimate" index.html && grep -q "estimateProcessingTime" index.html && echo "Estimate element and function exist"</verify>
  <done>Processing time estimate displays based on audio duration and file size</done>
</task>

<task type="auto">
  <name>Task 2: Add log callback to AudioProcessingService and visual progress bar</name>
  <files>src/services/audioProcessingService.js, index.html</files>
  <action>
PART A - Modify AudioProcessingService to emit log messages via progress callback:

In src/services/audioProcessingService.js, find the FFmpeg log handler in _processAudioInternal (the on('log') handler).
After capturing to this.lastLogs array, also call onProgress with log:

```javascript
// In the log handler, after: this.lastLogs.push({ type, message });
if (onProgress) {
  onProgress({ stage: 'log', message: message });
}
```

PART B - Add visual progress bar and FFmpeg log panel to index.html:

1. Replace the existing processing-status div with structured progress display:
   ```html
...
5. Update progress callback to handle logs and update progress bar:
   ```javascript
...
6. Clear logs when starting new processing (at start of Export Audio handler):
   ```javascript
   elements.ffmpegLogs.innerHTML = '';
   logsVisible = false;
   elements.ffmpegLogs.style.display = 'none';
   elements.toggleLogsBtn.textContent = 'Show Logs';
   ```

7. Reset progress bar in finally block:
   ```javascript
   elements.processingProgressFill.style.width = '0%';
   ```

8. Update success/error/cancel status handling to use processingText instead of processingStatus.textContent:
   - Success: `elements.processingText.textContent = 'Download started: ' + downloadFilename;`
   - Error/Cancel: Update processing-status container styling AND processingText content
  </action>
  <verify>grep -q "toggle-logs-btn" index.html && grep -q "processing-progress-fill" index.html && grep -q "stage: 'log'" src/services/audioProcessingService.js && echo "Progress bar, log panel, and log callback exist"</verify>
