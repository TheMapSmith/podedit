...
Purpose: Deliver the core user-facing cut marking functionality. User can mark start/end points, see regions highlighted in transcript, and view a list of all marked regions.
...
4. Click "Delete" on cut item - cut removed from list
  </verify>
...
highlightCutRegions(cutRegions) {
  ...
  segmentElements.forEach((element, index) => {
    ...
    const isInCut = cutRegions.some(cut => {
      if (!cut.isComplete || (typeof cut.isComplete === 'function' && !cut.isComplete())) {
        return false;
      }
...
3. Segments can show both "active" (current playback) and "in-cut-region" states
  ...
  <done>TranscriptController has highlightCutRegions method that visually marks segments within cut regions</done>
</task>

<task type="auto">
  <name>Task 3: Wire transcript highlighting to cut changes</name>
  <files>index.html</files>
  <action>
Ensure the onCutListChanged callback properly triggers transcript highlighting.

The callback is already wired in Task 1, but verify:
1. After transcript is generated, if there are existing cuts, highlight them
2. When cuts change, re-highlight

Add this after `transcriptController.generateTranscript()` call completes:
```javascript
// After transcript generation completes, highlight any existing cuts
// This handles the case where user marks cuts, then regenerates transcript
```

Actually, the existing wiring from Task 1 handles this. Just verify the flow works:
- onCutListChanged calls transcriptController.highlightCutRegions(cuts)
- This runs whenever cuts are added/removed

One addition needed: When transcript is initially rendered, check for existing cuts:
In the file load handler, after showing cut section, add:
```javascript
// If there are existing cuts (rare but possible), highlight them when transcript renders
```

This is handled by the onCutListChanged callback firing when cuts change. The transcript must exist for highlighting to work, which is ensured by the guard clause in highlightCutRegions.
  </action>
  <verify>
Full flow test:
1. Load audio file
2. Generate transcript
3. Mark start at 10 seconds
4. Mark end at 30 seconds
5. Observe: transcript segments between 10-30s have yellow highlight
6. Delete the cut
7. Observe: highlighting removed
  </verify>
  <done>Cut region highlighting fully wired between CutController and TranscriptController</done>
</task>
...
- Cut regions are visually highlighted in transcript with yellow/gold background (CUT-03 partial)
- User can see list of all cut regions with timestamps (CUT-03 partial)
